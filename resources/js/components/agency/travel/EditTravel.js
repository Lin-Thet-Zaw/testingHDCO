...
function EditTravel({isShow, isToggle, travelId, travelData}) {
    ...
    useEffect(async()=>{
        ...
        try {
            ...
            await axios.get(`/agency/travel/${travelId}/edit`)
            .then(res=>{
                setFormAddress(res.data.trip_addresses)
    ...
    const updateTravel = async(e)=>{
        ...
        try {
            ...
            await axios.put(`/agency/travel/${travelId}`, inputData)
            .then(res=>{
                travelData(res.data)
  ...
  >
    <Modal.Header closeButton>
      <Modal.Title id="example-modal-sizes-title-lg text-center">
        Edit Trip {loadingUpdate && (<Spinner animation="grow" />)}
      </Modal.Title>
    </Modal.Header>
    <Modal.Body>
        {
            loading || !inputData ? <div className="text-center"><Spinner animation="grow" /></div>:(
                <div className="card">
                ...
                formAddress && formAddress.map((element, index)=>(
                    ...
                    <Form.Control type="text" placeholder="Enter location" name="location" value={element.location} onChange={e=>handleFormChange(index, e)} />
                        {errors && errors.location && (<Form.Text className="text-danger">
                            {errors.location}
                        </Form.Text>)}
                    </Form.Group>
                    ...
                    <Row className="mb-3">
                        ...
                        <Form.Control type="date" placeholder="Enter location" name="started_date" value={inputData.started_date} onChange={handleInput} />
                    ...
                    <Form.Group className="mb-3" controlId="exampleForm.ControlTextarea1">
                        ...
                        <Form.Control as="textarea" rows={3} placeholder="Enter travel description" name="description" value={inputData.description} onChange={handleInput} />
                        {errors && errors.description && (<Form.Text className="text-danger">
                                {errors.description}
                            </Form.Text>)}
                    </Form.Group>
                    {
                        loadingUpdate?<Button variant="primary" disabled>
                        <Spinner
                          as="span"
                          animation="grow"
                          size="sm"
                        ...
                        />
                        Loading...
                      </Button>:<Button variant="primary" type="submit" onClick={updateTravel}>
                        UPDATE <i class="bi bi-upload"></i>
                    </Button>
                    }
